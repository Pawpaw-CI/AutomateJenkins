...
class Jobs(object):
    ...
    def create(self, job_name, config):
        ...
        if job_name in self:
            job_url = "%s/config.xml" % [ job_row for job_row in self._data if job_row['name'] == job_name][0]['url']
            self.jenkins.requester.post_xml_and_confirm_status(url=job_url, data=config)
